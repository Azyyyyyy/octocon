...
/// <summary>
/// Hosts the full scenario implementations for API authentication, WebSocket protocol,
/// domain push events, friendship fanout, OAuth callback, and front history integration tests.
/// Each public method maps 1-to-1 to a named test in one of the focused wrapper fixtures:
/// </summary>
public class ApiAuthLegacyScenarios
{
    [Test, ApiIntegration]
    public async Task Api_AuthAndIdempotencyFlow_WorksInDevHeaderMode()
    {
        ...
        var first = await SendAlterCreateAsync(client, idempotencyKey, "IntegrationOne");
        ...
        var second = await SendAlterCreateAsync(client, idempotencyKey, "IntegrationOne");
    ...
    private static Uri? WebSocketBasePath(HttpClient client)
    {
    ...
    public async Task Api_UserSocketEndpoint_AllowsWebSocketUpgrade_WithToken()
    {
        ...
        var uri = new Uri(WebSocketBasePath(client) + $"/api/socket?token={socketToken}");
    ...
    public async Task Api_OAuthCallback_IssuesJwsCompactSerializationToken()
    {
        ...
        Ensure(root.TryGetProperty("iss", out var iss) && iss.GetString() == "test-authority",
            $"Expected payload iss=test-authority. Payload: {Encoding.UTF8.GetString(payloadBytes)}");

        Ensure(root.TryGetProperty("sub", out var sub) && !string.IsNullOrWhiteSpace(sub.GetString()),
            $"Expected non-empty payload sub. Payload: {Encoding.UTF8.GetString(payloadBytes)}");
    ...
    public async Task Api_UserSocketEndpoint_PushesFrontingChangedEvent_AfterFrontStart()
    {
        ...
        Ensure(endpointAck is not null, "Expected endpoint ack (phx_reply) for front start call.");
        Ensure(frontingChangedPush is not null, "Expected fronting_changed push after front start call.");
    ...
    public async Task Api_UserSocketEndpoint_PushesFriendRequestReceived_ToRecipientSystem()
    {
        ...
        var socketUri = new Uri(WebSocketBasePath(client) + $"api/socket?token={socketToken}");
